...
def load():
    ...
    with open('data/conf', 'rb') as file:
...
def text_to_speech(text):
    ...
    try:
        pygame.mixer.music.load("data/empty.mp3")
    except:
        pass
...
toleranceslider = tk.Scale(root, from_=40, to=-40, width=15, length=88, orient=HORIZONTAL, bg=seconderycolor,
                           command=settollerance, showvalue=0)
# Set its value
toleranceslider.set(sensint)
...
### Check button ###
checkbutton = tk.Button(root, text="CHECK", command=check, width=12, height=3)
checkbutton.place(relx=0.99, rely=0.73, anchor='e')
